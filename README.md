...
2. Копируем jar на наш VPS переименуй его одновременно в bot.jar: 'scp build/libs/WordsTelegramBot-1.0-SNAPSHOT-all.jar
...
##### Обзор
Telegram бот теперь использует интерфейс с одним сообщением, где:
- Меню появляется при запуске бота
- Просмотр статистики заменяет меню при нажатии
- Кнопка "Назад" возвращает к меню
- Сброс прогресса остается в представлении статистики с кнопкой "Назад"
- Вся навигация происходит в рамках одного сообщения